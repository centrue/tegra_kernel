...
static int vc_mod_setup(struct vc_ctrl *ctrl, int mod_i2c_addr, struct vc_desc *desc)
{
        ...
        /* Validate descriptor-controlled bounds before any mode iteration. */
        if (desc->num_modes == 0 || desc->num_modes > ARRAY_SIZE(desc->modes)) {
                vc_err(dev_mod, "%s(): Invalid module mode count %u! Operation not possible!\n",
                        __FUNCTION__, desc->num_modes);
                ctrl->client_mod = NULL;
                i2c_unregister_device(client_mod);
                return -EINVAL;
        }
        if (desc->bytes_per_mode != sizeof(struct vc_desc_mode)) {
                vc_err(dev_mod, "%s(): Invalid module mode size %u (expected %zu)!\n",
                        __FUNCTION__, desc->bytes_per_mode,
                        sizeof(struct vc_desc_mode));
                ctrl->client_mod = NULL;
                i2c_unregister_device(client_mod);
                return -EINVAL;
        }
        for (index = 0; index < desc->num_modes; index++) {
                ...
                if ((mode->num_lanes != 1 && mode->num_lanes != 2 &&
                     mode->num_lanes != 4) ||
                    (mode->format != FORMAT_RAW08 &&
                     mode->format != FORMAT_RAW10 &&
                     mode->format != FORMAT_RAW12 &&
                     mode->format != FORMAT_RAW14)) {
                        vc_err(dev_mod,
                                "%s(): Invalid mode %d (lanes=%u format=0x%02x)!\n",
                                __FUNCTION__, index, mode->num_lanes,
                                mode->format);
                        ctrl->client_mod = NULL;
                        i2c_unregister_device(client_mod);
                        return -EINVAL;
...
int vc_core_init(struct vc_cam *cam, struct i2c_client *client,
        __u8 controller_i2c_addr, __u8 sensor_i2c_addr,
        __u16 force_mod_id, bool forced_mod_id,
        bool configure_sensor_address)
{
        struct vc_desc *desc = &cam->desc;
        struct vc_ctrl *ctrl = &cam->ctrl;
        struct device *dev = &client->dev;
        ...
        int configured_sensor_addr;
        int ret;

        if (forced_mod_id && !vc_mod_id_supported(force_mod_id)) {
                vc_err(dev, "Forced VC MIPI module profile 0x%04x is unsupported\n",
                        force_mod_id);
                return -EINVAL;
        }

        cam->controller_i2c_addr = controller_i2c_addr;
        cam->sensor_i2c_addr = sensor_i2c_addr;
        cam->forced_mod_id = forced_mod_id;
        cam->configure_sensor_address = configure_sensor_address;
        ctrl->client_sen = client;
        ret = vc_mod_setup(ctrl, controller_i2c_addr, desc);
        if (ret) {
                return ret;
        }

        cam->detected_mod_id = desc->mod_id;
        cam->selected_mod_id = forced_mod_id ? force_mod_id : desc->mod_id;
        detected_name = vc_mod_id_name(cam->detected_mod_id);
        selected_name = vc_mod_id_name(cam->selected_mod_id);

        if (!detected_name) {
                vc_err(dev, "Detected VC MIPI module 0x%04x is unsupported\n",
                        cam->detected_mod_id);
                ret = -ENODEV;
                goto release_module;
        }

        vc_notice(dev, "Detected VC MIPI module: %s (0x%04x)\n",
                detected_name, cam->detected_mod_id);
        if (forced_mod_id) {
                vc_notice(dev, "Forced VC MIPI profile: %s (0x%04x)\n",
                        selected_name, cam->selected_mod_id);
                if (cam->detected_mod_id != cam->selected_mod_id)
                        vc_warn(dev,
                                "Detected module %s differs from forced profile %s; model-specific writes require matching hardware\n",
                                detected_name, selected_name);
        }
        vc_notice(dev, "Using VC MIPI control profile: %s (0x%04x)\n",
                selected_name, cam->selected_mod_id);
        vc_notice(dev, "VC MIPI I2C addresses: controller=0x%02x sensor=0x%02x\n",
                controller_i2c_addr, sensor_i2c_addr);

        configured_sensor_addr = i2c_read_reg(&ctrl->client_mod->dev,
                ctrl->client_mod, MOD_REG_SEN_ADDR, __FUNCTION__);
        if (configured_sensor_addr < 0) {
                ret = configured_sensor_addr;
                goto release_module;
        }
        if (configure_sensor_address && configured_sensor_addr != sensor_i2c_addr) {
                vc_notice(dev,
                        "Updating VC controller sensor address from 0x%02x to 0x%02x\n",
                        configured_sensor_addr, sensor_i2c_addr);
                ret = i2c_write_reg(&ctrl->client_mod->dev, ctrl->client_mod,
                        MOD_REG_SEN_ADDR, sensor_i2c_addr, __FUNCTION__);
                if (ret)
                        goto release_module;
                configured_sensor_addr = i2c_read_reg(&ctrl->client_mod->dev,
                        ctrl->client_mod, MOD_REG_SEN_ADDR, __FUNCTION__);
                if (configured_sensor_addr != sensor_i2c_addr) {
                        vc_err(dev,
                                "VC controller sensor address verification failed: expected 0x%02x, got 0x%x\n",
                                sensor_i2c_addr, configured_sensor_addr);
                        ret = configured_sensor_addr < 0 ? configured_sensor_addr : -EIO;
                        goto release_module;
                }
        } else if (!configure_sensor_address && configured_sensor_addr != sensor_i2c_addr) {
                vc_err(dev,
                        "VC controller routes sensor address 0x%02x but DT reg is 0x%02x; add vc,sensor-address to configure it explicitly\n",
                        configured_sensor_addr, sensor_i2c_addr);
                ret = -EINVAL;
                goto release_module;
        }

        ret = vc_mod_ctrl_init(ctrl, desc, cam->selected_mod_id);
        if (ret) {
                goto release_module;
        }
...
void vc_core_release(struct vc_cam *cam)
{
        if (cam && cam->ctrl.client_mod) {
                i2c_unregister_device(cam->ctrl.client_mod);
                cam->ctrl.client_mod = NULL;
        }
}
...
int vc_sen_set_roi(struct vc_cam *cam)
{
        ...
        if (ctrl->flags & FLAG_PREGIUS_S) {
                ...
                if (MOD_ID_IMX412 == cam->selected_mod_id) {
