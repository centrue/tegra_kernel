...
#if defined(VC_MIPI_JETSON_NANO) && (defined(VC_MIPI_L4T_32_7_4) || defined(VC_MIPI_L4T_32_7_5) || defined(VC_MIPI_L4T_32_7_6))
...
void vc_init_tegra_controls(struct tegracam_device *tc_dev) 
{
        ...
        for (i = 0; i < tcc_handler->numctrls; i++) {
                ...
                if (TEGRA_CAMERA_CID_TRIGGER_MODE == ctrl->id) {
                        vc_notice(dev, "%s(): Overwrite control trigger_mode (min: %d, max: %d, default: %d)\n", __FUNCTION__, 0, 8, 0);
                        err = v4l2_ctrl_modify_range(ctrl, 0, 8, 1, 0);
                        if (0 > err) {
                                vc_err(dev, "%s(): Could not overwrite trigger_mode control!\n", __FUNCTION__);
                        }
                }
        }
}
...
static int vc_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
        ...
        __u32 controller_i2c_addr = 0x10;
        __u32 sensor_i2c_addr = client->addr;
        __u32 force_mod_id = 0;
        bool forced_mod_id;
        ...
        of_property_read_u32(dev->of_node, "vc,controller-address",
                &controller_i2c_addr);
        configure_sensor_address = !of_property_read_u32(dev->of_node,
                "vc,sensor-address", &sensor_i2c_addr);
        forced_mod_id = !of_property_read_u32(dev->of_node,
                "vc,force-mod-id", &force_mod_id);

        if (controller_i2c_addr > 0x7f || controller_i2c_addr < 0x08) {
                vc_err(dev, "%s(): Invalid VC controller I2C address 0x%x\n",
                        __func__, controller_i2c_addr);
                return -EINVAL;
        }
        if (sensor_i2c_addr > 0x7f || sensor_i2c_addr < 0x08) {
                vc_err(dev, "%s(): Invalid sensor I2C address 0x%x\n",
                        __func__, sensor_i2c_addr);
                return -EINVAL;
        }
        if (sensor_i2c_addr != client->addr) {
                vc_err(dev,
                        "%s(): vc,sensor-address 0x%x must match DT reg 0x%x\n",
                        __func__, sensor_i2c_addr, client->addr);
                return -EINVAL;
        }
        if (controller_i2c_addr == sensor_i2c_addr) {
                vc_err(dev, "%s(): Controller and sensor I2C addresses collide at 0x%x\n",
                        __func__, controller_i2c_addr);
                return -EINVAL;
        }
        if (force_mod_id > 0xffff) {
                vc_err(dev, "%s(): Forced module ID 0x%x is out of range\n",
                        __func__, force_mod_id);
                return -EINVAL;
        }

        ret = vc_core_init(cam, client, controller_i2c_addr,
                sensor_i2c_addr, force_mod_id, forced_mod_id,
                configure_sensor_address);
        if (ret) {
                vc_err(dev, "%s(): Error in vc_core_init: %d\n", __func__, ret);
                return ret;
        }
        vc_init_io(dev, cam);
