	...
	struct sensor_image_properties *image)
{
	...
	err = read_property_u32(node, "active_l",
		&image->left);
	if (err) {
		/* Existing NVIDIA camera mode nodes predate crop-origin data. */
		image->left = 0;
		dev_dbg(dev, "%s:active_l missing, defaulting to zero\n",
			__func__);
	}
	...
	if (err) {
		image->top = 0;
		dev_dbg(dev, "%s:active_t missing, defaulting to zero\n",
			__func__);
	}
